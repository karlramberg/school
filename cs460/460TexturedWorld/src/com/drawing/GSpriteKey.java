...
public class GSpriteKey implements GShape {
	...
	enum KEYBOARD {
		UP, DOWN, RIGHT, LEFT, RUN, WALK, JUMP
	}

	int animationType;
	long startFrameTime = 0;
	long interval = 100; // how fast the animation frames are changing
	...
	private Texture levelCompleteTexture;
	...
	GSpriteKey(final GL2 gl, int x, int y, long interval, float w, float h) {
		...
		this.levelCompleteTexture = GTextureUtil.loadSimpleTexture("/world/levelComplete.png");
	...
	public void calculateSpriteCollision() {
		...
		for (GCRect rect : this.collisionRects) {
			...
			if (inFlag) {
				...
				/// if the dino reached the end
				if(inRect.getLowerLeftPoint().x == 171f) {
					levelCompleteTimer = 320;
				}
	...
	public void render(final GL2 gl) {
		...
		if(levelCompleteTimer > 0) {
			...
			levelCompleteTexture.enable(gl);
			levelCompleteTexture.bind(gl);
			
			gl.glBegin(GL_TRIANGLE_STRIP);
			gl.glTexCoord2f(1, 0);
			gl.glVertex2f(64f, -32f); // v0 bottom right
			gl.glTexCoord2f(1, 1);
			gl.glVertex2f(64f, 32f); // v1 top right
			gl.glTexCoord2f(0, 0);
			gl.glVertex2f(-64f, -32f); // v2 bottom left
			gl.glTexCoord2f(0, 1);
			gl.glVertex2f(-64f, 32f); // v3 top left
			gl.glEnd();	
